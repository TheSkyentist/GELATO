...
def verify(params):
    ...
    for p in ['OutFolder', 'VacuumWav', 'RandomSeed', 'ContinuumRegion', 'LineRegion', 'NBoot', 'FThresh', 'NProcess', 'Plotting', 'FlamUnits', 'CalcEW', 'Overwrite', 'Concatenate', 'Verbose', 'EmissionGroups']:
    ...
    for p in params.keys():
        ...
        if p in ['OutFolder','FlamUnits']:
            ...
            if not (((type(params[p]) == float) or (type(params[p]) == int)) and ((params[p] >= 0) and (params[p] <= 1))):
                ...
                return False
        elif p in ['VacuumWav','Plotting','CalcEW','Concatenate','Overwrite','Verbose']:
